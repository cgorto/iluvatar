...
use crate::{
    CameraId, CameraIntrinsics, CameraPose, CoordinateMode, TrackedObject, VoxelContribution,
};
        ...
        match self {
            ...
            MotionPixelIter::RunLength {
                ...
                loop {
                    ...
                    {
                        let pixel =
                            MotionPixel::new(run.x_start + *current_offset, run.y, run.intensity);
                        *current_offset += 1;
                        return Some(pixel);
                    }
                    // Move to next run
...
mod tests {
    use super::*;
    use crate::{
        CameraIntrinsics, CameraPose, CoordinateMode, DistortionModel, Fov, GeoPosition,
        LocalizationStatus, PoseUncertainty, VoxelContribution,
    };
    ...
    fn test_motion_data_from_motion_pixels() {
        let input = vec![(100u32, 200u32, 50u8), (300, 400, 100)];
    ...
    fn test_enum_discriminants_stable() {
        ...
        let heartbeat = CameraMessage::Heartbeat {
            camera_id: 0,
            timestamp: 0,
        };
    ...
    // =========================================================================
    // Odin Wire Compatibility Reference Bytes
    // =========================================================================

    /// Reference test values shared between Rust and Odin. Uses fixed values
    /// so the Odin encoder can compare byte-for-byte.
    fn odin_test_intrinsics() -> CameraIntrinsics {
        CameraIntrinsics {
            focal_length: Vec2::new(600.0, 600.0),
    ...
    fn odin_test_pose() -> CameraPose {
        CameraPose {
            position: GeoPosition::new(47.6, -122.3, 10.0),
            orientation: Quat::IDENTITY,
            timestamp: 1000000,
            uncertainty: PoseUncertainty {
                position_stddev: glam::Vec3::splat(1.0),
                orientation_stddev: glam::Vec3::splat(0.01),
            },
            status: LocalizationStatus::Nominal,
        }
    }
    ...
    #[test]
    fn test_odin_reference_register() {
        let msg = CameraMessage::Register(CameraRegistration {
            version: 2,
            camera_id: 42,
            intrinsics: odin_test_intrinsics(),
            initial_pose: odin_test_pose(),
        ...
        eprintln!("REGISTER_LEN={}", bytes.len());
    ...
    #[test]
    fn test_odin_reference_motion() {
        let msg = CameraMessage::Motion(MotionFrame {
            camera_id: 42,
            sequence: 1,
            timestamp: 1000000,
            pose: odin_test_pose(),
    ...
    #[test]
    fn test_odin_reference_heartbeat() {
        let msg = CameraMessage::Heartbeat {
            camera_id: 42,
            timestamp: 2000000,
        };
        ...
        eprintln!("HEARTBEAT_LEN={}", bytes.len());

        let decoded: CameraMessage = deserialize(&bytes).unwrap();
        match decoded {
            CameraMessage::Heartbeat { camera_id, .. } => assert_eq!(camera_id, 42),
            _ => panic!("Expected Heartbeat"),
        }
    }

    #[test]
    fn test_odin_reference_server_registered_with_prefs() {
        let msg = ServerMessage::RegisteredWithPrefs {
            camera_id: 42,
            preferences: ServerPreferences {
                preferred_format: FrameFormat::MotionPixels,
        ...
        let bytes = serialize(&msg).unwrap();
        eprintln!(
            "REGISTERED_PREFS_HEX={}",
            bytes
                .iter()
                .map(|b| format!("{:02x}", b))
                .collect::<String>()
        );
        eprintln!("REGISTERED_PREFS_LEN={}", bytes.len());
    }

    #[test]
    fn test_odin_reference_server_grid_config() {
        let msg = ServerMessage::GridConfig(GridConfigMessage {
            origin_lat: 47.6,
            origin_lon: -122.3,
            origin_alt: 0.0,
        ...
        eprintln!("GRID_CONFIG_LEN={}", bytes.len());
    }
}
