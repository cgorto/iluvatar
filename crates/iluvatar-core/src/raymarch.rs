use crate::{
    BoundingBox, CameraIntrinsics, CameraPose, CoordinateMode, GeoPosition, MotionPixel, Ray,
    RaymarchConfig, VoxelContribution, ray_aabb_intersection, safe_div,
};
...
impl Raymarcher {
    ...
    /// Convert a camera position to local grid coordinates.
    ///
    /// In GPS mode, uses WGS84 → ECEF → ENU transform relative to the grid origin.
    /// In local mode, treats the GeoPosition fields as (x, y, z) meters and
    /// returns the offset from the grid origin directly.
    fn position_to_local(&self, position: &GeoPosition) -> Vec3 {
        match self.coordinate_mode {
            CoordinateMode::Gps => position.to_local_enu(&self.world_origin),
            CoordinateMode::Local => position.to_local_meters(&self.world_origin),
        }
    }
    ...
    pub fn raymarch_motion_pixels(
        ...
        let origin = self.position_to_local(&pose.position);
...
mod tests {
    use super::*;
    use crate::{
        CameraPose, CoordinateMode, DistortionModel, Fov, LocalizationStatus,
        MAX_CONTRIBUTIONS_PER_FRAME, MotionPixel, PoseUncertainty, Timestamp,
    };
    ...
    #[test]
    fn test_raymarch_into_matches_raymarch_motion_pixels() {
        // Verify the zero-alloc drain path (raymarch_into) produces identical
        // (key, intensity) pairs as the Vec-collecting path (raymarch_motion_pixels).
        // This is the core equivalence guarantee the server relies on.
        let mut raymarcher_a = test_raymarcher();
        let mut raymarcher_b = test_raymarcher();
        let pose = test_pose_with_orientation(Quat::IDENTITY);

        let pixels = vec![
            MotionPixel::new(500, 300, 100),
            MotionPixel::new(960, 540, 200),
            MotionPixel::new(1400, 800, 150),
            MotionPixel::new(200, 100, 80),
        ];

        // Path A: raymarch_into collects (key, intensity) via closure.
        let mut sink_entries: HashMap<u64, f32> = HashMap::new();
        raymarcher_a.raymarch_into(&pose, &pixels, &mut |key, intensity| {
            sink_entries
                .entry(key)
                .and_modify(|v| *v += intensity)
                .or_insert(intensity);
        });

        // Path B: raymarch_motion_pixels returns Vec<VoxelContribution>.
        let vec_result = raymarcher_b.raymarch_motion_pixels(&pose, &pixels);
        let mut vec_entries: HashMap<u64, f32> = HashMap::new();
        for contrib in &vec_result {
            let key = pack_voxel_key(contrib.index.x, contrib.index.y, contrib.index.z);
            vec_entries
                .entry(key)
                .and_modify(|v| *v += contrib.intensity)
                .or_insert(contrib.intensity);
        }

        // Both paths must produce the same set of voxel keys.
        assert_eq!(
            sink_entries.len(),
            vec_entries.len(),
            "Key count mismatch: raymarch_into={}, raymarch_motion_pixels={}",
            sink_entries.len(),
            vec_entries.len(),
        );

        for (key, sink_intensity) in &sink_entries {
            let vec_intensity = vec_entries
                .get(key)
                .expect("raymarch_into produced a key not found in raymarch_motion_pixels");
            assert!(
                (sink_intensity - vec_intensity).abs() < 1e-4,
                "Intensity mismatch at key {}: into={}, vec={}",
                key,
                sink_intensity,
                vec_intensity,
            );
    ...
    #[test]
    fn test_raymarch_into_empty_input() {
        // Empty pixel slice must never invoke the sink.
        let mut raymarcher = test_raymarcher();
        let pose = test_pose_with_orientation(Quat::IDENTITY);

        let mut call_count: u32 = 0;
        raymarcher.raymarch_into(&pose, &[], &mut |_key, _intensity| {
            call_count += 1;
        });

        assert_eq!(call_count, 0, "Sink should never be called for empty input");
    }
}
