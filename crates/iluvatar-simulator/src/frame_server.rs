//! TCP frame server for streaming rendered camera frames to external camera processes.
//!
//! ## Architecture
//!
...
fn run_tcp_acceptor(camera_id: u32, port: u16) {
    ...
    let listener = match TcpListener::bind(&addr) {
        Ok(l) => {
            info!("Frame server: camera {} listening on {}", camera_id, addr);
            l
        }
        Err(e) => {
            error!(
    ...
    for stream_result in listener.incoming() {
        match stream_result {
            ...
            Err(e) => {
                warn!("Frame server: camera {} accept error: {}", camera_id, e);
            }
        }
    }
}
